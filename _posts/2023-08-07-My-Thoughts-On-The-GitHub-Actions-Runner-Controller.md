...
The [GitHub Actions Runner Controller (ARC)][about-arc-docs] is a Kubernetes-based solution for hosting GitHub Actions Self-Hosted Runners within a containerized environment.  It follows the [Kubernetes Operator pattern][kubernetes-operator-pattern], which essentially consists of a controller workload that monitors the Kubernetes cluster and manages a set of Kubernetes workloads based on [Custom Resource Definitions (CRDs)][kubernetes-crds-doc].
